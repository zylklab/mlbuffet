...
Please make sure that the model file generated by train.py ends up located in the "/" directory. Take into account that these code will be executed inside a containerized environment and the resulting model must be able to be located by the Inferrer to auto-upload it into the system.